...
#                        Last Modified : Di 05 Mai 2020 19:45:17 CEST
...
reinitializeShell() {
  su - "${USERNAME}" -c ". ~/.profile" &> /dev/null || error "Could not source .profile"
  su - "${USERNAME}" -c ". ~/.zshrc" &> /dev/null || error "Could not source .zshrc"
}
